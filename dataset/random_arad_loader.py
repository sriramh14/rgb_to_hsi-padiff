...
def normalize_hsi_cube(hsi: np.ndarray) -> np.ndarray:
    """Normalize an HSI cube to [0, 1] so it matches RGB / 255 scaling."""
    hsi = np.nan_to_num(
    ...
    if hsi_min < 0.0 or hsi_max > 1.0:
        hsi = hsi / hsi_max
    return np.clip(hsi, 0.0, 1.0).astype(np.float32, copy=False)
...
class RandomARAD1KDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        hsi = normalize_hsi_cube(mat[self.cube_key])
